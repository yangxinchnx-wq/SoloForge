...
public class SystemPromptBuilder {
    ...
    private String buildBehaviorLayer() {
        return """
            ...
            你可以调用 delegateToSubModel 工具将子任务委托给副模型执行。
            适用场景:
            - 用户请求包含多个不同领域的子任务 (如前端+后端)
            - 某个子任务更适合用特定模型 (如 DeepSeek-Coder 写代码)
            - 需要并行处理多个独立子任务

            工作流程:
            1. 先调用 listSubModels 查看可用的副模型
            2. 调用 delegateToSubModel(modelName, task, agentId) 委托子任务
            3. 副模型执行完毕后返回结果
            4. 你整合所有副模型的结果,输出给用户

            注意: 简单任务自己处理即可,不需要委托。只有复杂任务才需要分解和委托。
