 ...
 *
 * 三层架构: 主模型(编排) → 副模型(worker, 自主选 agent) → Java agent(执行)
 *
 * 每个 SubModelWorker 代表一个副模型 worker:
 *   1. 用自己 LLM 配置调 AgentSelector 选 agent
...
public class SubModelWorker {
    ...
    public Flux<String> executeAsWorker(
        ...
        return Flux.defer(() -> {
            try {
                // 1. 副模型选 agent
                String agentId = agentSelector.selectAgent(message, subProvider);
                AgentIdentityEntity agent = agentRepo.findById(agentId)
                    .orElseThrow(() -> new RuntimeException("Agent not found: " + agentId));

                // 2. 发 SSE agent 事件 (前端流送区显示 "副模型 → agent → 任务")
                ...
                return agentExecutor.executeStream(message, workerSettings, subProvider, history, fileContext);
            } catch (Exception e) {
    ...
    public Flux<String> executeWithMainModel(
        ...
        return Flux.defer(() -> {
            try {
                ...
                return agentExecutor.executeStream(message, settings, mainProvider, history, fileContext);
            } catch (Exception e) {
