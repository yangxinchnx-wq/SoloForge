...
describe('SoloForge Layer 4 Rust 高性能调度内核跨语言管道集成测试套件', () => {
  ...
  it('验证点 2：[跨语言全生命周期重排自愈] 连续打入不同权重智能体任务，验证真实时间轴下的 Aging 老化反超机制', async () => {
    ...
    // 2. 等待足够时间让 aging 生效（老化速度：5000分/秒）
    //    需要等待 > (50-5)/5000 = 9ms 即可反超，这里等待 200ms 确保生效
    await delay(200);

    // 3. 推入一个初始分极低（仅5分）、但老化速度极快（每秒飙升5000分）的饥饿任务
    await client.pushTask('patient_task_b', 5, 5000.0);

    // 4. 等待足够时间让 B 的 aging 超过 A（需要 >45/5000 = 9ms）
    await delay(200);

    // 5. 物理弹出：时间自愈起效，B 节点必须突破压制、率先被弹出！
