 ...
 *
 * 使用方式:
 *   <StreamContextMenu x={...} y={...} onClose={() => ...} />
 */
import React, { useCallback, useEffect, useRef } from 'react';
import { Check, X, RotateCcw } from '../utils/icons';
...
// ── 预设字号 ──
const PRESET_SIZES: number[] = [8, 9, 10, 11, 12, 14, 16];
...
export const StreamContextMenu = React.memo(function StreamContextMenu({ x, y, onClose }: StreamContextMenuProps) {
  ...
  return (
    <div
      ref={menuRef}
      className="fixed z-50 w-[220px] rounded-xl bg-surface border border-outline/40 shadow-[0_8px_32px_rgba(0,0,0,0.28)] overflow-hidden"
    ...
    >
      ...
      <div className="px-3 py-2 space-y-1.5">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[9px] text-on-surface/30 font-mono shrink-0">8</span>
          <input
            type="range"
            min={8}
            max={18}
          ...
          />
          <span className="text-[9px] text-on-surface/30 font-mono shrink-0">18</span>
        </div>
        {/* 预设字号 */}
