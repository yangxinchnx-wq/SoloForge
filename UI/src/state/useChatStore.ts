  ...
  handleSend: async (_inputRef) => {
    ...
    const chatHandle = await startChat({ chatId: activeChatId, prompt: finalContent, mode: permissionMode, history: activeMessages.map(m => ({ sender: m.sender, content: m.rawContent || m.content })), fileContext: selectedFile ? { name: selectedFile, content: editorContent } : undefined, mainProvider: { baseUrl: mainEntry.baseUrl, apiKey: mainEntry.apiKey, model: mainEntry.model, rateLimitProfile: (mainEntry as any).rateLimitProfile || null }, subProviders: subEntries.map(e => ({ baseUrl: e.baseUrl, apiKey: e.apiKey, model: e.model, rateLimitProfile: (e as any).rateLimitProfile || null })), candidateProviders: candidateEntries.map((e: any) => ({ displayName: e.model, providerName: e.providerName, modelName: e.model, baseUrl: e.baseUrl })), ...(hashlineAgentEnabled ? { toolCallMode: 'hashline' } : {}), workspaceFolder: useChatsStore.getState().getChat(activeChatId)?.workspaceFolder, activeTools: activeTools.length > 0 ? activeTools : undefined, activeSkills: activeSkills.length > 0 ? activeSkills : undefined, activeKnowledge: activeKnowledge.length > 0 ? activeKnowledge : undefined, activeSettings: configs[activeChatId] || fallbackActiveSettings, toolSchemas: toolSchemas.length > 0 ? toolSchemas : undefined, canvasId: getCanvasSessionId(activeChatId) } as any, async (evt: ChatStreamEvent) => {
      switch (evt.kind) {
        ...
        case 'agent': { streamBridge.onAgent(evt.agentId, evt.name, evt.avatar, evt.mainModel, evt.subModels, evt.role, evt.domain, evt.subModel); break; }
        case 'usage': { streamBridge.onUsage(evt.usage); break; }
        case 'error': { if (!streamFinalized) { streamFinalized = true; streamBridge.onError(evt.error); } activeStreamContext = null; const fm = classifyStreamError(evt.error || ''); { const lmsg = uiMessageStore.getLastAssistantMessage(activeChatId); if (lmsg) uiMessageStore.completeMessage(activeChatId, lmsg.id, 'error'); } set((s) => { const cl = s.conversations[activeChatId] || []; if (cl.length === 0) return { isGenerating: false, isPaused: false, activeChatHandle: null, streamState: { ...emptyStreamState } }; const nl = [...cl]; const lm = { ...nl[nl.length - 1] }; if (lm.sender === 'assistant') lm.content = `\u274C **AI 调用失败**：${fm}`; nl[nl.length - 1] = lm; return { conversations: { ...s.conversations, [activeChatId]: nl }, isGenerating: false, isPaused: false, activeChatHandle: null, streamState: { ...emptyStreamState } }; }); break; }
        case 'done': {
          // ★ FIX: 如果已被 pause/abort 定格, 跳过 done 处理 — 防止清空 activeStreamContext 导致无法恢复
          if (streamFinalized) break;
          streamFinalized = true; streamBridge.onDone(evt.agentId); { const lmsg = uiMessageStore.getLastAssistantMessage(activeChatId); if (lmsg) uiMessageStore.completeMessage(activeChatId, lmsg.id, 'done'); } activeStreamContext = null; set({ isGenerating: false, isPaused: false, activeChatHandle: null, streamState: { ...emptyStreamState } });
          const expFp = (evt as any).experienceFingerprint as string | undefined;
  ...
  handleAcceptEnable: async (candidateName) => {
    ...
    startChat({ chatId: activeChatId, prompt: '', mode: lastReqBody.mode, history: [], mainProvider: (lastReqBody.mainProvider as any), subProviders: newSub ? [...(lastReqBody.subProviders as any[]), newSub] : (lastReqBody.subProviders as any[]), candidateProviders: (lastReqBody.candidateProviders as any[]).filter((c: any) => c.modelName !== candidateName), activeTools: lastReqBody.activeTools, activeSkills: lastReqBody.activeSkills, activeKnowledge: lastReqBody.activeKnowledge, toolSchemas: lastReqBody.toolSchemas, activeSettings: configs[activeChatId] || fallbackActiveSettings } as any, (evt: ChatStreamEvent) => {
      switch (evt.kind) {
        ...
        // ★ 2026-07-20: 补齐 completeMessage + isGenerating:false, 与 handleSend/resumeChat 保持一致
        case 'error': { streamBridge.onError(evt.error); { const lmsg = uiMessageStore.getLastAssistantMessage(activeChatId); if (lmsg) uiMessageStore.completeMessage(activeChatId, lmsg.id, 'error'); } set({ isGenerating: false, isPaused: false, activeChatHandle: null, streamState: { ...emptyStreamState } }); break; }
        case 'done': { streamBridge.onDone(); { const lmsg = uiMessageStore.getLastAssistantMessage(activeChatId); if (lmsg) uiMessageStore.completeMessage(activeChatId, lmsg.id, 'done'); } if (canvasPusher2) canvasPusher2.flush().catch(() => {}); set({ isGenerating: false }); break; }
